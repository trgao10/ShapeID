...
idx1 = IndsTBtoTaxaCode(strcmpi(Infraorder, 'Lemuriformes'));
idx2 = IndsTBtoTaxaCode(strcmpi(Infraorder, 'Lorisiformes'));
idx3 = IndsTBtoTaxaCode(strcmpi(IGroup, 'Plesiadapiform, "proto-primate"'));
...
maxDistortion = max(squareform(distMatrix)./pdist(Y)) * max(pdist(Y)./squareform(distMatrix));
disp(['max distortion: ' num2str(maxDistortion)]);
LSDistortion = mean(squareform(abs(distMatrix.^2 - squareform(pdist(Y)).^2)));
disp(['LS distortion: ' num2str(LSDistortion)]);
axis equal
hold on
...
%%%% save embedding coordinates in a JSON file
%%%%%% a cell array of structs

% TaxaCodeIdx = [idx1; idx2; idx3];
% CTableIdx = [find(strcmpi(Infraorder, 'Lemuriformes'));...
%              find(strcmpi(Infraorder, 'Lorisiformes'));...
%              find(strcmpi(IGroup, 'Plesiadapiform, "proto-primate"'))];
embeddings = cell(1,size(distMatrix, 1));
for j=1:size(distMatrix, 1)
    embeddings{j} = struct('Name', taxa_code{IndsTBtoTaxaCode(j)},...
                           'Order', Order{j}, 'Family', Family{j},...
                           'Genus', Genus{j}, 'IGroup', IGroup{j},...
                           'Chirality', Chirality{j},...
                           'Infraorder', Infraorder{j},...
