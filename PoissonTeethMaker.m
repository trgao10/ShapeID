...
for j=1:length(GroupNames)
    ...
    for k=1:numRandMeshes
        ...
        if (k==1)
            [reconMeshList{k},ORT,PSD] = PoissonMeshInterpolation(domainMesh, ReparametrizedMeshList, Weights{k});
        else
            reconMeshList{k} = PoissonMeshInterpolation(domainMesh, ReparametrizedMeshList, Weights{k}, ORT, PSD);
        end
        disp([num2str(k) '/' num2str(numRandMeshes) ' done.']);
    end
    save([GrouPath 'reconMeshes.mat'], 'reconMeshList', 'Weights');
