...
load('data/LipmanSDP.mat')
NumTeeth = size(distMat, 1);
distMat(isinf(distMat)) = 0;
distMatrix = (distMat+ distMat')/2;
...
meanDistortion = mean(squareform(distMatrix)./pdist(Y)) * mean(pdist(Y)./squareform(distMatrix));
set(gcf, 'Name', ['Average Distance Distortion = ' num2str(meanDistortion)]);
scatter(Y(idx1,1),Y(idx1,2),30,[1,0,1],'filled');
axis equal
hold on
scatter(Y(idx2,1),Y(idx2,2),30,[0,1,0],'filled');
scatter(Y(idx3,1),Y(idx3,2),30,[0,0,1],'filled');
legend('Lemuriformes', 'Lorisiformes', 'Proto-Primates', 'Location', 'best');
cameratoolbar;
cameratoolbar('SetCoordSys', 'none');
